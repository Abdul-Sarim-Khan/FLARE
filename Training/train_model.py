...
def main():
    ...
    # ── Dual-model bundle ───────────────────────────────────────────────────
    # cicids_model  → used by client for Network + DNS logs
    # botsv3_model  → used by client for System + PowerShell logs
    # When only one dataset is provided the other key is None,
    # so the client falls back to rule-only for that log type.
    # When both are trained, drop the new pkl on the client and done.

    cicids_model_data = None
    botsv3_model_data = None

    if args.cicids:
        # All vectors from CICIDS are network type (v[0] == 2.0)
        cicids_model_data = {
            "autoencoder": global_ae.get_weights(),
            "scaler": {"min": scaler.min_.tolist(), "max": scaler.max_.tolist()},
            "trained_on": "CICIDS2017",
            "log_types": ["network", "dns"],
        }

    if args.botsv3:
        # All vectors from BOTSv3 are system type (v[0] == 1.0)
        # When botsv3 is added later, it gets its own model entry
        botsv3_model_data = {
            "autoencoder": global_ae.get_weights(),
            "scaler": {"min": scaler.min_.tolist(), "max": scaler.max_.tolist()},
            "trained_on": "BOTSv3",
            "log_types": ["system", "powershell"],
        }

    model_bundle = {
        # New dual-model format
        "cicids_model":  cicids_model_data,
        "botsv3_model":  botsv3_model_data,
