# -*- coding: utf-8 -*-
"""
FLARE - Network Inference Engine
─────────────────────────────────────
...
def _batch_timestamp(df: pd.DataFrame) -> str:
    """
    Use the most recent flow-start time in this batch as the alert timestamp so
    alerts reflect when the traffic actually happened rather than when the
   (possibly backlogged) CSV row was scored. Falls back to wall-clock now when
    the FlowStartTime column is absent (older CSVs) or unparseable.
    """
    if "FlowStartTime" in df.columns:
        try:
            mx = pd.to_numeric(df["FlowStartTime"], errors="coerce").max()
            if pd.notna(mx) and mx > 0:
                return datetime.fromtimestamp(
                    float(mx), timezone.utc
...
def run_once(
    ...
    # Fix 9: Drop rows that carry no behavioural signal — corrupt/padding rows
    # and header-only stub flows (e.g. a lone SYN/RST with zero duration). The
    # MLP is biased toward ATTACK on near-empty vectors. DestinationPort and
    # Protocol are EXCLUDED from the "is this row empty?" test, so a flow
    # described only by its port (zero bytes, packets and timing) is treated as
    # degenerate too — that was the source of the low-confidence
    # "Network Attack Detected" / "SSH Brute Force" false positives on RPC
    # (port 135) and port-scan stub flows.
    _signal_cols = [c for c in feature_names if c not in ("DestinationPort", "Protocol")]
    _signal      = X_raw[_signal_cols].values if _signal_cols else X_raw.values
    nonzero_mask = (_signal != 0).any(axis=1)
    n_zero_rows  = int((~nonzero_mask).sum())
    if n_zero_rows:
        log.debug("net_infer: dropping %d signal-less rows (corrupt/stub flows)", n_zero_rows)
    ...
    if n_mlp_attacks > 0:
        ...
        if n_mlp_attacks <= _BURST_THRESHOLD:
            ...
            # Derive window boundaries from the actual flow start times in the batch
            _fst = pd.to_numeric(attack_rows.get("FlowStartTime", pd.Series(dtype=float)),
                                 errors="coerce").dropna()
            if len(_fst):
                _win_start = datetime.fromtimestamp(float(_fst.min()), timezone.utc).isoformat(timespec="seconds")
                _win_end   = datetime.fromtimestamp(float(_fst.max()), timezone.utc).isoformat(timespec="seconds")
            else:
                _win_start = _win_end = ts
            ...
            alert.window_start  = _win_start
            alert.window_end    = _win_end
...
class _NetInferWorker(threading.Thread):
    ...
    def __init__(
        ...
        self._current_date  = None          # tracks which day's file is open
        self._tracker       = None          # _OffsetTracker, rebuilt on date change
    ...
    def _get_tracker(self):
        """Return the _OffsetTracker for today's CSV, creating a new one if the date rolled over."""
        from datetime import date as _date
        today = _date.today()
        if self._current_date != today:
            csv_p              = self._csv_dir / f"net_flows_{today}.csv"
            self._tracker      = _OffsetTracker(csv_p)
            self._current_date = today
            log.info("net_infer: tracking %s", csv_p.name)
...
def start(
    ...
    Args:
        ...
        csv_dir:       Directory containing date-named flow CSVs
                      (e.g. net_flows_2026-06-09.csv).  Preferred over csv_path.
        csv_path:      Legacy: path to a fixed CSV file.  If csv_dir is also
                       supplied csv_path is ignored.  Falls back to the parent
                       directory of FLARE_NET_CSV env var.
    ...
    if csv_dir is not None:
        ...
        csv_p = Path(csv_path).parent
    else:
        default = os.environ.get("FLARE_NET_CSV", str(_PROJECT_DIR / "net_flows.csv"))
        csv_p = Path(default).parent
    ...
    log.info(
        "net_infer: started — csv_dir=%s  interval=%ds  model=%s",
...
def _cli():
    ...
    parser = argparse.ArgumentParser(
        description="FLARE — Network Inference Engine"
    )
