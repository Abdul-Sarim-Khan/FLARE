...
class _RuleWorker(threading.Thread):
    ...
    def __init__(
        ...
        self._cmdline_warned = False  # one-shot warning for missing 4688 command lines
    ...
    def _evaluate(self, event: dict):
        # Self-check: a 4688 with no command line means the host's audit policy
        # isn't capturing command lines, so shadow-copy / PowerShell rules can't
        # fire. Surface it (counter + one-shot warning) instead of silently
        # under-detecting. Counter is reported via heartbeat stats.
        if event.get("event_id") == 4688 and not str(event.get("CommandLine", "")).strip():
            self._counters["cmdline_missing"] = self._counters.get("cmdline_missing", 0) + 1
            if not self._cmdline_warned:
                self._cmdline_warned = True
                log.warning(
                    "HostEngine: 4688 process-creation events have an EMPTY command "
                    "line — command-line dependent rules (shadow-copy deletion, "
                    "PowerShell cradle/encoded execution) cannot fire. Enable "
                    "'Include command line in process creation events' "
                    "(ProcessCreationIncludeCmdLine_Enabled=1) and restart the agent."
                )
