...
# Values Windows writes into IpAddress for non-network (local / console) logons.
_NON_ROUTABLE_SRC = {"-", "", "::", "::1", "0.0.0.0", "127.0.0.1", "localhost"}


def _resolve_src(event: dict) -> tuple[str, bool]:
    """Resolve a usable source identifier for a 4625 logon-failure event.

    Returns (label, is_network_ip). The naive ``event.get("IpAddress", fallback)``
    is wrong: Windows *populates* IpAddress with the literal "-" for local/console
    logons, so the dict key exists and the fallback never triggers — producing
    remediation text like "Block -". Resolve explicitly: prefer a real network
    IP, then the workstation name, then a clearly-labelled local marker.
    """
    ip = (event.get("IpAddress") or "").strip()
    if ip.lower() not in _NON_ROUTABLE_SRC:
        return ip, True
    ws = (event.get("WorkstationName") or "").strip()
    if ws and ws != "-":
        return ws, False
    return "local console", False
...
def check_brute_force(event: dict, ioc: IOCLoader, state: RuleState) -> Optional[RuleResult]:
    ...
    if event["event_id"] != 4625:
        return None
    src_ip, is_net = _resolve_src(event)
    username = event.get("TargetUserName", "")

    # Brute force = many failures against the SAME account from one source.
    # Key on (src_ip, username) so a password-spray wave (one attempt per
    # account) does not inflate this counter — that pattern is owned by
    # check_password_spray. Local LogonUser network logons report
    # IpAddress="-", so without the username every failed logon across all
    # rules would otherwise collapse into a single shared bucket.
    total, distinct = state.record(f"brute:{src_ip}:{username}", username)

    if total >= 10:
        # Fix 5: suppress duplicate alerts — only fire once per WINDOW_SECS
        # burst to avoid N-threshold alerts for a single attack wave.
        if not state.check_and_record_alert(f"brute:{src_ip}:{username}"):
            return None
        action = (
            f"Block or isolate {src_ip}. " if is_net
            else f"Source is a local/console logon ({src_ip}); review interactive "
                 "sessions and scheduled tasks on this host. "
        )
        return RuleResult(
...
def check_password_spray(event: dict, ioc: IOCLoader, state: RuleState) -> Optional[RuleResult]:
    ...
    # Spray is defined by breadth (many DISTINCT accounts), not raw volume.
    # Record the username under one key per source; `distinct` is the headline
    # metric. `total` is the window-wide failed-logon count from this source and
    # is reported as supporting context only — it should not be read as
    # "spray attempts", since a co-located brute-force wave inflates it.
    ...
    if distinct >= 5 and total >= 5:
        ...
        if not state.check_and_record_alert(f"spray:{src_ip}"):
            return None
        action = (
            f"Block {src_ip} immediately. " if is_net
            else f"Source is a local/console logon ({src_ip}); audit interactive "
                 "sessions and any automation running on this host. "
        )
        return RuleResult(
            ...
            evidence = {
                ...
                "window_failures":   total,   # all failed logons from src in window (context)
                "last_username":     username,
            },
        )
    return None
...
def check_scheduled_task_suspicious(event: dict, ioc: IOCLoader, state: RuleState) -> Optional[RuleResult]:
    ...
    if event["event_id"] != 4698:
        return None
    # Event 4698 carries the full task definition XML in the "TaskContent"
    # field. The previous "TaskContentXml" key does not exist on the event, so
    # this rule only ever saw the task *name* and never matched the action
    # command line (powershell/hidden/temp paths) — meaning the suspicious-task
    # alert never fired, only the generic check_scheduled_task_any one.
    task_content = (event.get("TaskContent")
                    or event.get("TaskContentXml")
                    or event.get("TaskName", ""))
...
def check_privileged_group_add(event: dict, ioc: IOCLoader, state: RuleState) -> Optional[RuleResult]:
    ...
    if event["event_id"] not in (4728, 4732, 4756):
        return None
    # Events 4728/4732/4756 put the GROUP name in TargetUserName (there is no
    # "GroupName" field on these events) and the added member in MemberName /
    # MemberSid. Reading "GroupName" meant the group was always "" and this
    # rule never fired — e.g. `net localgroup Administrators <u> /add` (4732)
    # went undetected.
    group_name = event.get("TargetUserName") or event.get("GroupName", "")
    member     = event.get("MemberName") or event.get("MemberSid", "")
    ...
    return RuleResult(
        ...
        suggestion   = (
            f"Verify that adding '{member or '?'}' to "
            f"'{group_name or '?'}' was authorised. "
