<#
.SYNOPSIS
    FLARE - Step 1: Grant OS privileges required by the agent.
...
Write-Host ""
Write-Host "  FLARE - Privilege Grant" -ForegroundColor White
...
# Inbound UDP 37020 so the agent (and the installer's discovery listener) can
# receive the server's LAN beacon. Without this, Windows Firewall drops the
# unsolicited inbound broadcast and server auto-discovery silently fails.
$beaconRule = "FLARE-Beacon-Inbound"
if (Get-NetFirewallRule -DisplayName $beaconRule -ErrorAction SilentlyContinue) {
    Write-Warn "Firewall rule '$beaconRule' already exists - skipping"
} else {
    try {
        New-NetFirewallRule `
            -DisplayName $beaconRule `
            -Direction   Inbound `
            -Protocol    UDP `
            -LocalPort   37020 `
            -Action      Allow `
            -Profile     Any `
            -Description "Allow FLARE agent to receive the server's UDP/37020 LAN discovery beacon" |
            Out-Null
        Write-OK "Firewall rule created (UDP inbound port 37020 - beacon discovery)"
    } catch {
        Write-Warn "Could not create beacon firewall rule: $_"
    }
}
