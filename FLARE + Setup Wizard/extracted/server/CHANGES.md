...
All changes made during the  development cycle, including every session of
...
All `` and `` version strings have been updated to ``:
...
`FLARE Endpoint Agent` were already correct in this version.

> **Note for uninstall:** if a legacy `FLARE Endpoint Agent` service is
